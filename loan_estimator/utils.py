...
import numpy as np
from sklearn.linear_model import LinearRegression#.ensemble import GradientBoostingRegressor

from loan_estimator.Item import Car
from loan_estimator.constants import *
from loan_estimator.website_parsers.ads_api.ads_api import ADS_API
from loan_estimator.website_parsers.crwl_api.crwl_api import CRWL_API
...
def get_price(car, list_of_cars):
    ...
    :return: float
    """
    gbr = LinearRegression()#GradientBoostingRegressor(loss='ls', max_depth=6)
    print("GB1")
...
def cars_to_json(cars:List[Car], best_price):
    ...
    for i, car in enumerate(cars):
        ...
        car_dict['Пробег'] = '-' if car.km == 0 else car.km
...
def car_from_json(params: dict):
    ...
    car.year = int(params.get('year', 0))
    car.km = int(params.get('mileage', 0))
    car.engine_type = params.get('enginetype', '')
    car.engine_volume = params.get('evolume', '')
    car.kpp = params.get('korobka', '')
    return car
